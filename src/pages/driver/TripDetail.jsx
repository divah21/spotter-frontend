...
import { ArrowLeft, MapPin, Clock, Fuel, Coffee, Navigation } from 'lucide-react'
    ...
    <div className="min-h-screen bg-linear-to-br from-slate-50 to-slate-100 py-8">
      <div className="container mx-auto px-4 max-w-7xl">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-6 flex items-center justify-between">
          ...
          <Badge variant="outline">Status: {trip.status}</Badge>
        </motion.div>
        ...
        <Card className="mb-6 border-primary/20 shadow-xl">
          ...
          <CardContent className="pt-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="space-y-3">
                <h3 className="font-semibold text-primary">Stops Summary</h3>
                <div className="space-y-2">
                  <div className="flex items-center gap-2 text-sm">
                    <Coffee className="w-4 h-4 text-orange-500" />
                    <span className="text-slate-600">Rest Stops:</span>
                    <Badge variant="secondary">{stops.filter(s => s.type === 'rest').length || 0}</Badge>
                  </div>
                  <div className="flex items-center gap-2 text-sm">
                    <Fuel className="w-4 h-4 text-yellow-500" />
                    <span className="text-slate-600">Fuel Stops:</span>
                    <Badge variant="secondary">{stops.filter(s => s.type === 'fuel').length || 0}</Badge>
                  </div>
                  <div className="flex items-center gap-2 text-sm">
                ...
                {stops.map((stop, index) => (
                  <div key={index} className="flex items-start gap-4 p-3 bg-slate-50 rounded-lg border border-slate-200">
                    ...
                    <div className="flex-1">
                      <div className="font-semibold text-primary capitalize">{stop.name || stop.type}</div>
                      <div className="text-sm text-slate-600">{stop.location || ''}</div>
                    </div>
                    <div className="text-right">
                      <div className="text-sm font-semibold text-slate-900">{stop.miles_from_start ?? 0} mi</div>
                      <div className="text-xs text-slate-500">{stop.time_label || 'Stop'}</div>
                    </div>
                  </div>
                ))}
              </div>
